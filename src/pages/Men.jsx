...
import MenSplitHero from '../components/MenSplitHero';
import MenPromoGrid from '../components/MenPromoGrid';
import MenBrandMarquee from '../components/MenBrandMarquee';
...
const Men = () => {
    ...
    const categoriesList = ['All', 'T-Shirts', 'Shirts', 'Jeans', 'Trousers', 'Jackets'];
    const sizes = ['S', 'M', 'L', 'XL', 'XXL'];

    useEffect(() => {
        let result = products.filter(p =>
            ['Tops', 'Bottoms', 'Outerwear'].includes(p.category) &&
            p.name.length % 2 !== 0
        ).slice(0, 12);
    ...
    const menTrendingProducts = products.filter(p => ['Tops', 'Bottoms', 'Outerwear', 'Footwear'].includes(p.category) && p.id % 2 !== 0).slice(0, 8);

    return (
        <div className="shop-page-standard" style={{ backgroundColor: '#fff' }}>

            <MenSplitHero />
            <MenPromoGrid products={products} />
            <MenBrandMarquee />

            <div style={{ padding: '0 1rem', background: '#fff', margin: '2rem 0' }}>
                <HorizontalProductScroll title="TRENDING IN MEN'S" products={menTrendingProducts} />
            </div>
